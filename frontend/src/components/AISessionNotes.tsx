...
export const AISessionNotes: React.FC<AISessionNotesProps> = ({ courseId, video, onSeek, onOpenKeySettings, onShowToast, initialTab = 'summary' }) => {
  ...
  const handleGenerateNotes = async () => {
    ...
    try {
      ...
      onShowToast(error instanceof Error ? error.message : 'Could not generate AI notes.');
      if (error instanceof Error && /api key|key is invalid|API_KEY_INVALID/i.test(error.message)) onOpenKeySettings();
    } finally {
      ...
      {!studyNotes ? (
        ...
        <>
          ...
          <div role="tabpanel" className="min-h-48 py-4 text-sm text-text-secondary">
            ...
            {activeTab === 'notes' && <div className="whitespace-pre-wrap leading-relaxed">{studyNotes.detailedNotes || 'No detailed notes were generated.'}</div>}
